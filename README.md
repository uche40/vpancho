...
You are currently reading the **Vuero Vue 3 1.0.0** documentation. The product uses:
...
- Typekit's [`webfontloader`](https://github.com/typekit/webfontloader) utility to load web fonts from any sources
...
The routes are defined in the [./src/router.ts](./src/router.ts) file.  
...
Some components names begin with `The`, like `TheSidebar.vue`. Those are inteded to have a single active instance.  
Also when referring to components inside the template, consider using `CamelCase` insted of `pascal-case` notation.

> Tip: You can find all recommandations and best practices in the Vue3 style guide  
